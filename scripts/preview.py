...
def invent(
    ...
    def sky(day: dt.date) -> random.Random:
        """Return the generator for that day's weather, shared by every lawn in the garden.
    ...
    entry_id = zone.get("entry_id", entry_id)
    field = {
        "name": name,
        "area_m2": float(zone.get("area_m2", 120.0 if entry_id == "preview" else 80.0)),
        "latitude": float(garden.get("latitude", 45.0)),
        "longitude": float(garden.get("longitude", 9.0)),
        "exposure": zone.get("exposure", "full_sun"),
        "soil_type": zone.get("soil_type", "loam"),
        "grass_type": zone.get("grass_type", "tall_fescue"),
        "establishment_method": zone.get("establishment_method", "sod"),
        "establishment_date": zone.get("establishment_date", "2025-04-12"),
        "irrigation_type": zone.get("irrigation_type", "pop_up_spray"),
        "flow_l_min": None,
        "precipitation_rate_mm_h": float(zone.get("precipitation_rate_mm_h", 15.0)),
        "valve_entity": zone.get("valve_entity", "switch.lawn_valve"),
        "mower_entity": zone.get("mower_entity"),
        "weather_entity": "weather.forecast_home",
        "features": [] if zone else [{"kind": "deciduous_tree", "shade_pct": 20.0}],
        "shaded_fraction": 0.0 if zone else 0.2,
        "application_rate_mm_h": float(zone.get("precipitation_rate_mm_h", 15.0)),
    }
    ...
    for offset in range(39, -1, -1):
        ...
        if weather and date in weather["tmax"] and date in weather["tmin"]:
            ...
            u2 = believable_wind(
                weather["wind"].get(date, 7.2) / 3.6 if date in weather["wind"] else None
            )
            rs = believable_sun(
                weather["solar"].get(date, 0.0) * 86400 / 1e6 if date in weather["solar"] else None,
        ...
        et0, _ = et.reference_et0(
            ...
            elevation_m=float(garden.get("elevation", 280)),
            day_of_year=doy,
        )
    ...
    result = assess.assess(
        ...
        weather=et.WeatherDay(
            ...
            rs_mj=et.solar_radiation_from_temperature(
                today_weather.tmax,
                today_weather.tmin,
                et.extraterrestrial_radiation(lawn.latitude, today.timetuple().tm_yday),
            ),
        ),
        forecast=ahead_days,
    ...
    sunrise = dt.datetime.combine(
        ...
        dt.time(6, 55),
        tzinfo=dt.timezone(dt.timedelta(hours=garden.get("timezone_offset_hours", 2))),
    )
    ...
    state = {
        ...
        "irrigation_needed_min": (
            round(result.needed_mm * lawn.minutes_per_mm) if lawn.minutes_per_mm else None
        ),
...
<script type="module">
  ...
  // One next-action sensor per lawn, named as Home Assistant would name it: that is how the
  // panel notices a field has been recomputed. Looking a lawn up by a fixed key is what left
  ...
  for (const lawn of lawns) {
    states[`sensor.${slug(lawn.field.name)}_next_action`] = {
      state: lawn.state.next_action,
      last_updated: new Date().toISOString(),
    };
    states[`sensor.${slug(lawn.field.name)}_activity`] = {
      state: lawn.state.activity || "idle",
      attributes: lawn.state.activity_details || {},
    };
  }
  const hass = {
    language: lang,
    ...
    states,
    async callWS(msg) {
      if (msg.type === "hosekeeper/fields") return Object.values(snapshots).map((s) => ({ entry_id: s.entry_id, name: s.field.name }));
      if (msg.type === "hosekeeper/field") return snapshots[msg.entry_id];
      if (msg.type === "hosekeeper/log") {
        ...
        const snapshot = snapshots[msg.entry_id];
        ...
        if (msg.what === "status") {
          ...
          snapshot.events = [
            ...
            {
              uid: `${msg.entry_id}:${today}:preview:${kind}`,
              date: today,
              start: null,
              end: null,
              all_day: true,
...
class Handler(SimpleHTTPRequestHandler):
    ...
    garden: ClassVar[dict[str, Any] | None] = None

    def do_GET(self) -> None:
        """Serve the snapshot, a frontend module, or the index."""
        if self.path.startswith("/api/snapshot"):
            zones = self.garden["zones"]
...
def _sample(name: str) -> Path:
    ...
    A real garden and a real station's weather say where somebody lives, so they are kept in
    `scripts/local/`, which git ignores. The preview picks them up without being asked, and
    falls back to a made-up garden under a made-up summer for everybody else.
    """
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="the lawns to show; defaults to scripts/local/garden.json if present, else the sample",
    )
    parser.add_argument(
        "--prefix",
    ...
    parser.add_argument(
        "--open-meteo",
        action="store_true",
        help="fetch fresh weather for the garden's coordinates instead of the saved sample",
    )
    parser.add_argument(
        "--weather",
        type=Path,
