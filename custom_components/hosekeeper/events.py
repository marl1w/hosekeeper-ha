...
def build(
    ...
    zone = {"zone": field.name, "zone_id": zone_id}
    ...
    for item in state.agenda:
        ...
        if item["code"] == "irrigate":
            if plan.get("date") == date and plan.get("cycles"):
                ...
                out.extend(
                    _cycle_event(zone_id, date, index, cycle, len(plan["cycles"]), plan, zone)
            ...
            if projected:
                out.extend(
                    _cycle_event(zone_id, date, index, cycle.as_dict(), len(projected), plan, zone)
            ...
            if decided:
                ...
                # this zone's place in the valve queue included. Without it every zone of a
                # sown lawn is drawn watering at eleven, which is one job three zones cannot
                # do at once and a promise the plan itself never makes.
        ...
        out.append(
            {
                "uid": f"{zone_id}:{date}:{item['code']}:{item['params'].get('operation', '')}",
                "date": date,
                "start": start,
                "end": end,
                "all_day": all_day,
    ...
    if plan.get("syringe") and plan.get("syringe_start"):
        out.append(
            {
                "uid": f"{zone_id}:{plan['date']}:syringe",
                "date": plan["date"],
    ...
    for op in state.plan:
        ...
        out.append(
            {
                "uid": f"{zone_id}:{op['month']}:{op['code']}",
                "date": first,
                "start": None,
                "end": None,
                "all_day": True,
...
def _cycle_event(
    ...
    return {
        "uid": f"{zone_id}:{date}:irrigate:{index}",
        "date": date,
        "start": cycle["start"],
        "end": cycle["end"],
        "all_day": False,
