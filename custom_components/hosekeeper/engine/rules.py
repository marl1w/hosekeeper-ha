...
def rule_mowing(ctx: Context) -> list[Advice]:
    ...
    if ctx.phenology.heat_stress or ctx.deficit_mm > ctx.raw_mm * 1.2:
        return [
            Advice(
                "delay_mowing_stress",
                "mowing",
                3,
                {"height_mm": high},
                ("heat_stress" if ctx.phenology.heat_stress else "drought_stress",),
            )
        ]
    height = high if ctx.phase == "summer_stress" else round((low + high) / 2)
    planned_codes = {op.code for op in ctx.planned("mowing")}
    if "raise_mowing_height" in planned_codes or ctx.shaded_fraction >= 0.3:
        height = high  # in shade the leaf needs all the light it can get
    if ctx.phase == "late_autumn" or "last_mow_lower" in planned_codes:
    ...
    if since is None:
        return [Advice("mow_soon", "mowing", 3, {"height_mm": height}, ("no_mowing_recorded",))]
    ...
    if since >= interval:
        return [
            Advice(
                "mow_soon", "mowing", 3, {"days": since, "height_mm": height}, ("interval_reached",)
            )
        ]
    return [
        Advice(
