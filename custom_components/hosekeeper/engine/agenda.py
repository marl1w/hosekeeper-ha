...
from . import et, plan, rules, schedule, water
...
def build(
    ...
    *,
    ...
    # A robot is scheduled here like anything else: the point of the engine is to decide when
    # the grass is best cut, not to hand that back to the machine. What changes is the
    # cadence, because a robot takes a few millimetres at a time and keeps up by going often
    # rather than by cutting hard. The same table the month plan uses, so the calendar and
    # the month's line can never say different things.
    # ...unless the rules are keeping the robot off new seed, in which case the cut is being
    ...
    # stretches -- and dropping the cuts on top of that left a hot fortnight with no mowing
    # dated at all, which for grass in the autumn flush is three weeks of growth to take off
    # in one pass. The day's advice says whether to hold; the calendar still says when it
    # falls due.
    ...
    for op in ctx.planned():
        if op.category in held_back:
            continue
        # Some of the month's lines are not jobs for a day. The milestones are settings
        # rather than work, and a routine runs all month: the cut it asks for is already on
        # the week as its own item, so dating the routine too put "mow every day" on a
        # Tuesday beside the Tuesday cut.
        if (
            op.category in ("general", "irrigation")
            or op.code in plan.ONGOING_CODES
            or op.code
            in (
                "first_mow",
                "raise_mowing_height",
