...
def current(
    *,
    ...
    """
    # Every state names the same two things in the same two places: which lawn this is, and
    # what to act on. One template then works for watering, cutting and everything after
    # them, which a `valve` key on one state and a `mower` key on another does not allow.
    base: dict[str, Any] = {"zone": zone, "config_entry_id": entry_id}
