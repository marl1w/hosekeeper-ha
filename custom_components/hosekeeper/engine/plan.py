    ...
    for year, month in months_ahead(ctx.today):
        ...
        for code, category, optional, params, basis in programme_year.get(folded, []):
            ...
            if category == "mowing":
                ...
                # While the new seed roots the cut is made by hand, so the month asks for a
                # push mower's interval and says why the robot is in its dock.
                robot_waiting = (
                    ctx.robot_mower
                    and then.days_since_sowing is not None
                    and then.days_since_sowing < programme.SEED_GERMINATION_DAYS
                )
                if robot_waiting and code == "mow_routine":
                    tailoring.append("robot_wheels_tear_seedlings")
                robot = ctx.robot_mower and code == "mow_routine" and not robot_waiting
                interval = (
                    programme.ROBOT_PASS_DAYS if robot else programme.MOW_INTERVAL_DAYS
                ).get(phase)
                if interval is None:
                    continue
                if robot:
                ...
                if code == "last_mow_lower":
                    height = low + (high - low) // 4
                elif phase == "summer_stress" or code == "raise_mowing_height":
                    height = high  # a longer leaf shades its own soil through the heat
                elif ctx.shaded_fraction >= 0.3:
                    height = high
                    tailoring.append("shade_needs_leaf")
                else:
                    height = round((low + high) / 2)
