...
def assess(
    ...
    *,
    ...
    kc = grass.crop_coefficient(lawn.grass_type, today.month, lawn.northern_hemisphere)
    ...
    for day_key, row in days.items():
        for item in row.get("maintenance", []):
            events.setdefault(day_key[:7], set()).add(item.get("type", ""))
    ...
    # reshuffle them; only their timing moves.
    month_key = key[:7]
    if plan_state.get("generated") != month_key:
...
def status_of(
    ...
    for key, record in days.items():
        for item in record.get("maintenance", []):
            events.setdefault(key[:7], set()).add(item.get("type", ""))
