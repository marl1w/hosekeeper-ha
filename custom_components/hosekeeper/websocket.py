...
def zones(hass: HomeAssistant):
    """Return every loaded zone of every lawn, with the lawn it belongs to."""
    return [(entry, zone) for entry in _entries(hass) for zone in entry.runtime_data.zones.values()]


def find_zone(hass: HomeAssistant, zone_id: str):
    """Return one zone by its id, or None. The panel and the services both come here."""
    for entry, zone in zones(hass):
        if zone.zone_id == zone_id:
            return entry, zone
    return None


def field_summary(entry, zone) -> dict[str, Any]:
    """Return what the zone switcher needs."""
    state = zone.coordinator.data
    return {
        "zone_id": zone.zone_id,
        "lawn": entry.title,
        "name": zone.field.name,
        "grass_type": zone.field.grass_type,
        "area_m2": zone.field.area_m2,
        "next_action": state.next_action if state else None,
        "status": state.status if state else None,
    }


def field_snapshot(hass: HomeAssistant, entry, zone) -> dict[str, Any]:
    """Return everything the panel shows for one zone."""
    field = zone.field
    state = zone.coordinator.data
    today = dt_util.now().date()
    days = []
    for date, record in zone.diary.recent(HISTORY_DAYS, until=today):
    ...
    return {
        ...
        "days": days,
        "adaptation": zone.diary.adaptation,
        "plan": zone.diary.plan.get("operations", []),
        "events": build_events(field, zone.zone_id, zone.diary, state, today, sunrise_hint(hass)),
...
def ws_fields(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict) -> None:
    """List the fields."""
    connection.send_result(msg["id"], [field_summary(e, z) for e, z in zones(hass)])


@websocket_api.websocket_command(
    {vol.Required("type"): f"{DOMAIN}/field", vol.Required("zone_id"): str}
)
@callback
def ws_field(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict) -> None:
    """Return one field's snapshot."""
    found = find_zone(hass, msg["zone_id"])
    if found is None:
        connection.send_error(msg["id"], websocket_api.ERR_NOT_FOUND, "No such lawn")
        return
    connection.send_result(msg["id"], field_snapshot(hass, *found))
    ...
    try:
        if what == "maintenance":
            ...
            await coordinator.async_log_irrigation(
                float(msg["minutes"]), replace_today=True, at=_when(msg)
            )
        elif what == "status":
