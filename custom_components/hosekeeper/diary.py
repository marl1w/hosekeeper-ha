...
class Diary:
    """Load, query and append to one field's diary."""

    def __init__(self, hass: HomeAssistant, entry_id: str) -> None:
        """Bind to the storage file for one config entry."""
        self._hass = hass
        self._store: Store[DiaryData] = Store(
            hass, DIARY_STORAGE_VERSION, f"{DIARY_STORAGE_KEY}.{entry_id}"
        )
    ...
    def add_irrigation(self, minutes: float, mm: float | None, source: str) -> None:
        """Add an irrigation run to today."""
        today = self.today()
    ...
    def add_maintenance(
        ...
        *,
        ...
        self.day(self.today_key(when)).setdefault("maintenance", []).append(record)
