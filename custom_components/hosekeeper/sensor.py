...
async def async_setup_entry(
    ...
    # One set per zone, each filed under the subentry that owns it, so Home Assistant hangs
    # the entities on that zone's device rather than on the lawn's.
    for zone_id, zone in entry.runtime_data.zones.items():
        async_add_entities(
            (HosekeeperSensor(zone.coordinator, description) for description in SENSORS),
