...
/* The zone filter, drawn only while one is on. It sits beside the tabs because it qualifies
   all of them, and the toolbar wraps, so on a phone it takes a line of its own rather than
   squeezing the tabs or the date. */
.filter-chip {
  display: inline-flex; align-items: center; gap: 7px; min-width: 0; max-width: 100%;
  padding: 5px 7px 5px 10px; border-radius: var(--hk-radius-pill);
  border: 1px solid color-mix(in srgb, var(--hk-accent) 45%, var(--hk-line));
  background: color-mix(in srgb, var(--hk-accent) 12%, transparent);
  color: var(--hk-text); font: inherit; font-size: 0.8rem; font-weight: 600; cursor: pointer;
}
.filter-chip:hover { background: color-mix(in srgb, var(--hk-accent) 20%, transparent); }
.filter-chip:focus-visible { outline: 2px solid var(--hk-accent); outline-offset: 2px; }
.filter-chip .zone { width: 15px; height: 15px; font-size: 0.62rem; }
.filter-chip .icon { --mdc-icon-size: 16px; width: 16px; height: 16px; color: var(--hk-text-dim); }
/* The name is the one part that may shrink, and it needs a floor of its own: a flex item
   whose content can be clipped is allowed to collapse to nothing, which is how a chip ends
   up showing a dot and a cross and no lawn at all. */
.filter-chip__name { min-width: 4ch; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
@media (max-width: 700px) {
  ...
  /* On a phone it takes the whole width instead, and the label takes what is left. There
     is nothing to keep in line with, so the date wraps rather than losing its month. */
  .datenav { margin-left: 0; width: 100%; }
  .datenav__label { width: auto; min-width: 0; flex: 1 1 auto; font-size: 0.88rem; white-space: normal; overflow: visible; text-overflow: clip; }
...
class HosekeeperPanel extends HTMLElement {
  ...
  /**
   * Home Assistant assigns `hass` on every state change anywhere in the instance.
   *
   * On a busy install that is many times a second, and redrawing here rebuilt the whole page
   * each time and replayed every section's entry animation with it: that is the flicker. The
   * panel draws from the snapshots it fetches, and the only thing it reads out of `hass` at
   * render time is the language, so that is the only change worth a redraw. New data arrives
   * through _maybeRefresh, which redraws when a lawn has actually been recomputed.
   */
  set hass(hass) {
    ...
    this._maybeRefresh();
    if (pickLanguage(previous) !== pickLanguage(hass) || pickLocale(previous) !== pickLocale(hass)) {
      this._render();
    }
  }
  ...
  set narrow(value) {
    // Home Assistant assigns this alongside hass. Redrawing when it has not moved is the
    // same waste as redrawing for somebody else's light switch.
  ...
  async _loadAll() {
    ...
    // The filter is remembered between visits, and a zone can be gone by the next one —
    // renamed, removed, or set up again from scratch. Filtering on an id nothing answers to
    // hides every lawn there is, which reads as a panel that never finishes loading.
    if (this._zone && !this._fields.some((f) => f.zone_id === this._zone)) {
      this._zone = null;
      this._remember("zone", null);
    }
  ...
  async _loadField(zoneId) {
    try {
      const snapshot = await this._api.field(zoneId);
      this._snapshots.set(zoneId, snapshot);
      this._stamps.set(zoneId, this._stamp(snapshot));
    } catch (err) {
      console.error("hosekeeper: field", zoneId, err);
    }
  }
  ...
  _maybeRefresh() {
    for (const [zoneId, snapshot] of this._snapshots) {
      const stamp = this._stamp(snapshot);
      if (stamp && stamp !== this._stamps.get(zoneId)) {
        this._stamps.set(zoneId, stamp);
        this._loadField(zoneId).then(() => this._render());
      }
    }
  }
  ...
  _visibleSnapshots() {
    const all = this._fields.map((f) => this._snapshots.get(f.zone_id)).filter(Boolean);
    return this._zone ? all.filter((s) => s.zone_id === this._zone) : all;
  }
  ...
  _zoneColour = (zoneId) => {
    const index = this._fields.findIndex((f) => f.zone_id === zoneId);
    return ZONE_COLOURS[(index < 0 ? 0 : index) % ZONE_COLOURS.length];
  };

  _snapshotFor = (event) => this._snapshots.get(event.zone_id);

  _zoneName = (zoneId) => this._fields.find((f) => f.zone_id === zoneId)?.name || "";

  // ------------------------------------------------------------------ actions

  /**
   * Move the view one step, and take the chosen day with it.
   *
   * The list under the calendar shows the selected day. Paging only moved the anchor, so the
   * grid went to October while the list below it stayed on a day in September: the two
   * halves of the screen were describing different weeks. The selection moves by the same
   * step, which keeps the weekday when paging weeks and the date when paging months.
   */
  _shift(step) {
    const anchor = parseDay(this._anchor);
    const selected = parseDay(this._selected);
    if (this._mode === "month") {
      anchor.setMonth(anchor.getMonth() + step);
      // The 31st of a month before a short one lands on the 1st of the next; clamping keeps
      // the selection inside the month the grid is actually showing.
      const day = Math.min(selected.getDate(), new Date(anchor.getFullYear(), anchor.getMonth() + 1, 0).getDate());
      selected.setFullYear(anchor.getFullYear(), anchor.getMonth(), day);
    ...
    this._selected = this._mode === "day" ? this._anchor : isoDay(selected);
    this._render();
  }
  ...
  _confirm = async (event, payload) => {
    const zones = event.zones || [event.zone_id];
    for (const zoneId of zones) {
      try {
        const snapshot = await this._api.log(zoneId, payload);
        this._snapshots.set(zoneId, snapshot);
        this._stamps.set(zoneId, this._stamp(snapshot));
      } catch (err) {
        console.error("hosekeeper: log", zoneId, err);
      }
    }
    this._render();
  };
  ...
  _rate = (zoneId, status) => this._confirm({ zone_id: zoneId }, { what: "status", status });

  /** Report something seen on a lawn. */
  _issue = (zoneId, issue) => this._confirm({ zone_id: zoneId }, { what: "issue", issue });

  /** Set how many minutes a lawn was watered for today. */
  _water = (zoneId, minutes, at) =>
    this._confirm({ zone_id: zoneId }, { what: "irrigation", minutes, at });

  /** Record a job nothing asked for. */
  _record = (zoneId, kind, details, at) =>
    this._confirm({ zone_id: zoneId }, { what: "maintenance", kind, details, at });

  /** Record a feed. The integration works the nitrogen out from the product and the rate. */
  _feed = (zoneId, product, dose, at) =>
    this._confirm({ zone_id: zoneId }, { what: "fertilizing", product, dose_g_m2: dose, at });

  _selectZone(zoneId) {
    this._zone = zoneId;
    this._remember("zone", zoneId);
    this._render();
  }
  ...
  /**
   * Say which zone is being shown, and offer the way back.
   *
   * The filter is set from the overview and then follows you into every other view and into
   * the next visit, so without it on screen a week with one lawn's cuts on it looks like a
   * week in which nothing else was due. It goes beside the tabs because it qualifies every
   * one of them, and the toolbar wraps, so a phone gives it a line rather than squeezing
   * the tabs. Nothing is drawn while all the zones are showing: there is nothing to say and
   * nothing to undo.
   */
  _filterChip(lang) {
    const s = strings(lang);
    const zone = this._zone ? this._fields.find((f) => f.zone_id === this._zone) : null;
    if (!zone) return null;
    return el(
      "button",
      {
        class: "filter-chip",
        title: s.ui.showAllZones,
        "aria-label": `${zone.name} — ${s.ui.showAllZones}`,
        onClick: () => this._selectZone(null),
      },
      zoneBadge(zone.zone_id, this._zoneColour, this._zoneName),
      el("span", { class: "filter-chip__name" }, zone.name),
      icon("mdi:close")
    );
  }
  ...
  _renderToolbar(lang, locale) {
    ...
    const chip = this._filterChip(lang);
  ...
  _doRender() {
    ...
    // The entry animation belongs to arriving somewhere, not to redrawing where you already
    // are. Pressing Done should not fade the whole page back in.
    const view = [this._mode, this._anchor, this._selected, this._zone].join("|");
    this._content.classList.toggle("content--still", view === this._view);
    this._view = view;
    ...
    if (!snapshots.length) {
      // Before the first answer this is honest; after it, the snapshots did not arrive, and
      // saying "loading" for the rest of the session says nothing anybody can act on.
      const body = this._loaded ? s.ui.couldNotLoad : s.ui.loading;
    ...
    const glance = el(
      ...
      renderNow(this._fields.map((f) => this._snapshots.get(f.zone_id)).filter(Boolean), {
        lang,
        locale,
        zoneColour: this._zoneColour,
        zoneName: this._zoneName,
    ...
    if (this._mode === "overview") {
      this._content.append(
        renderOverview(this._fields.map((f) => this._snapshots.get(f.zone_id)).filter(Boolean), snapshots, byDay, {
          ...shared,
          todayIso: today,
          selectedZone: this._zone,
          onSelectZone: (id) => this._selectZone(id),
    ...
    // The month view and the overview are about the shape of the season, so they keep the
    // plan's standing lines. The day and the week are lists of work, and what is due in them
    // has been dated already.
    const work = this._mode === "month" ? byDay : datedOnly(byDay);
    const calendar =
      this._mode === "month"
        ? renderMonthGrid(byDay, this._anchor, this._selected, today, shared)
        : this._mode === "week"
          ? renderAgenda(work, this._anchor, this._selected, today, shared)
          : null;
