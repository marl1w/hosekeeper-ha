...
export function lawnChoice(snapshots, { lang, zoneName, id }) {
  ...
  return el(
    ...
    many ? el("option", { value: ALL }, s.ui.allLawns) : null,
    snapshots.map((snapshot) =>
      el("option", { value: snapshot.entry_id }, snapshot.field?.name || zoneName(snapshot.entry_id))
    )
  );
}
...
export function chosen(snapshots, select) {
  return select.value === ALL ? snapshots.map((s) => s.entry_id) : [select.value];
}
