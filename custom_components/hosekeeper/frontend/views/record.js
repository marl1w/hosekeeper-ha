...
export function recordDialog(
  ...
  const plannedFeed = (zoneId) =>
    (byId.get(zoneId)?.state?.plan || []).find(
  ...
  const sync = () => {
    ...
    if (what === "fertilizing") {
      const planned = first ? plannedFeed(first.zone_id) : null;
    ...
    onSubmit: () => {
      ...
      for (const zoneId of chosen(snapshots, lawn)) {
        ...
        if (what === "fertilizing") {
          ...
          onFeed(zoneId, product.value, Number(dose.value) || undefined, at);
          continue;
        }
        onRecord(zoneId, what, detailsFor(what), at);
      }
      return true;
    },
  });
