...
export function eventTitle(event, lang, locale, snapshot) {
  ...
  // The last resort still fills: an operation's name can carry its own height and interval,
  // and reaching it by this path is no reason to print the braces at the reader.
  return fill(lookup(lang, "operations", event.code), params, locale);
}
 ...
 */
export function zoneBadge(zoneId, zoneColour, zoneName) {
  const name = (zoneName ? zoneName(zoneId) : "") || "";
  ...
  const initial = distinctive.charAt(0).toUpperCase() || zoneId.charAt(0).toUpperCase();
  return el("span", { class: "zone", style: { background: zoneColour(zoneId) }, title: name }, initial);
}
