...
async def test_remove_deletes_the_file(hass: HomeAssistant) -> None:
    diary = Diary(hass, "zone4")
    await diary.async_load()
    diary.set_status("poor")
    await diary.async_save()
    await diary.async_remove()

    again = Diary(hass, "zone4")
    await again.async_load()
    assert again.days == {}


async def test_recording_the_same_job_again_amends_it(hass: HomeAssistant) -> None:
    """The panel has no other way to add what was left out the first time."""
    diary = Diary(hass, "zone5")
    await diary.async_load()
    when = dt.datetime(2026, 8, 30, 9, 0, tzinfo=dt.UTC)
    diary.add_maintenance("sowing", at=when)
    diary.add_maintenance("sowing", at=when, details={"seed_mix": "festuca and poa"})
    diary.add_maintenance("sowing", at=when, details={"rate_g_m2": 25})

    entries = diary.day("2026-08-30")["maintenance"]
    assert len(entries) == 1
    assert entries[0]["details"] == {"seed_mix": "festuca and poa", "rate_g_m2": 25}


async def test_two_machine_runs_in_a_day_are_two_records(hass: HomeAssistant) -> None:
    """A robot that goes out twice really did, and each pass has its own length."""
    diary = Diary(hass, "zone6")
    await diary.async_load()
    morning = dt.datetime(2026, 8, 30, 9, 0, tzinfo=dt.UTC)
    evening = dt.datetime(2026, 8, 30, 18, 0, tzinfo=dt.UTC)
    diary.add_maintenance("mowing", at=morning, source="mower", details={"duration_min": 45})
    diary.add_maintenance("mowing", at=evening, source="mower", details={"duration_min": 30})

    entries = diary.day("2026-08-30")["maintenance"]
    assert [e["details"]["duration_min"] for e in entries] == [45, 30]
