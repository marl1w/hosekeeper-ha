...
def _mid_afternoon(freezer) -> None:
    """Run in the same afternoon, for the same reason the coordinator suite does."""
...
async def _setup(
    ...
    lawn, zone = split(field_data | {"name": name}, lawn_name=name)
    entry = make_entry(hass, lawn, [zone], title=name)
...
async def test_fields_and_snapshot(
    ...
    assert result["result"][0]["zone_id"] == only_zone(entry).zone_id

    await client.send_json_auto_id(
        {"type": "hosekeeper/field", "zone_id": only_zone(entry).zone_id}
    )
    result = await client.receive_json()
    assert result["success"]
    ...
    await client.send_json_auto_id({"type": "hosekeeper/field", "zone_id": "nope"})
    result = await client.receive_json()
...
async def test_calendar_entity_lists_the_plan_and_the_log(
    ...
    await hass.services.async_call(
        DOMAIN,
        "log_maintenance",
        {"zone_id": only_zone(entry).zone_id, "kind": "mowing", "height_mm": 60},
        blocking=True,
    )
    await hass.async_block_till_done()
...
async def test_the_panel_can_confirm_a_job_and_gets_the_fresh_snapshot_back(
    ...
    assert only_zone(entry).diary.today()["maintenance"][0]["type"] == "mowing"
    ...
    result = await client.receive_json()
    assert result["success"], result
    assert only_zone(entry).diary.today()["irrigation_min"] == 25
    ...
    result = await client.receive_json()
    assert result["success"], result
    assert only_zone(entry).diary.today()["status"] == "good"
...
async def test_confirming_is_refused_for_a_lawn_that_is_not_there(
    ...
    client = await hass_ws_client(hass)
    await client.send_json_auto_id(
        {"type": "hosekeeper/log", "zone_id": "nope", "what": "maintenance", "kind": "mowing"}
    )
    result = await client.receive_json()
    assert not result["success"]
    assert result["error"]["code"] == "not_found"
...
async def test_a_feed_recorded_from_the_panel_counts_toward_the_nitrogen_budget(
    ...
    logged = only_zone(entry).diary.today()["maintenance"][0]
...
async def test_marking_the_seedbed_watering_done_closes_it(
    hass: HomeAssistant, hass_ws_client: WebSocketGenerator, field_data: dict[str, Any]
) -> None:
    """Done has to be visible, or it is a button that does nothing as far as anybody can see.

    The seedbed's passes are recorded as work rather than as irrigation -- they wet the top
    centimetre and the balance must not be told the roots were filled. Nothing read that
    record back, so the line went on asking for the watering after it had been confirmed.
    """
    entry = await _setup(hass, field_data)
    zone = only_zone(entry)
    zone.diary.add_maintenance("sowing", at=dt_util.now() - dt.timedelta(days=3))
    await zone.coordinator.async_refresh()
    await hass.async_block_till_done()
    assert "germination_watering" in [a["code"] for a in zone.coordinator.data.advice]
    ...
    assert "germination_watering" not in [a["code"] for a in state["advice"]]
    today = dt_util.now().date().isoformat()
    assert not [
        item
