...
make preview ARGS=--open-meteo      # the real weather over the garden in scripts/example-garden.json
make preview ARGS="--stats stats.json"   # a real station's own record
make preview DATE=2027-04-10        # pretend it is spring
```

`scripts/example-garden.json` describes the lawns the preview shows: their size, exposure,
soil, when the turf was laid and the measured precipitation rate. Point `--garden` at your
own copy to see your property. With `--open-meteo` the weather is a reanalysis of what
...
produce in Home Assistant. The panel is read-only: rating and logging go through the entities and services.
